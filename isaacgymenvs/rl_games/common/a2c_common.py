...
class A2CBase(BaseAlgorithm):
    ...
    def write_stats(self, total_time, epoch_num, step_time, play_time, update_time, a_losses_left, c_losses_left,
                    ...
                    last_lr_left, lr_mul_left,frame, scaled_time, scaled_play_time, curr_frames):
...
class ContinuousMultiA2CBase(A2CBase):
    ...
    def play_steps_multi(self):
        ...
        for n in range(self.horizon_length):
            if self.use_action_masks:
                ...
                res_dict_left = self.get_action_values_left(self.obs)
            ...
            actions_new = self.action_combine(res_dict_left['actions'], res_dict_left['actions'])
    ...
    def train_epoch_multi(self):
        ...
        with torch.no_grad():
            ...
            if self.is_rnn_left:  # Todo rewrite play_steps_rnn_multi
                batch_dict_left = self.play_steps_rnn_multi()  # evaluion: the interaction
            else:
                batch_dict_left = self.play_steps_multi()
        ...
        for mini_ep in range(0, self.mini_epochs_num):
            ...
            for i in range(len(self.dataset_left)):
                a_loss_left, c_loss_left, entropy_left, kl_left, last_lr_left, lr_mul_left, cmu_left, csigma_left, b_loss_left= self.train_actor_critic_multi(
                    self.dataset_left[i])
    ...
    def train(self):
        ...
        while True:
            ...
            a_losses_left, c_losses_left, b_losses_left, entropies_left, kls_left, last_lr_left, lr_mul_left= self.train_epoch_multi()
            ...
            if self.rank == 0:
                ...
                self.write_stats(total_time, epoch_num, step_time, play_time, update_time, a_losses_left, c_losses_left,
                                 entropies_left, kls_left, last_lr_left, lr_mul_left, frame, scaled_time,
                ...
                if self.game_rewards_left.current_size_left > 0:
                    ...
                    if self.save_freq > 0:
                        if (epoch_num % self.save_freq == 0) and (
                                mean_reward_left[0] <= self.last_mean_rewards):
                            self.save(os.path.join(self.nn_dir, 'last_' + checkpoint_name))

                    if mean_reward_left[0] > self.last_mean_rewards and epoch_num >= self.save_best_after:
                        print('saving next best left rewards: ', mean_reward_left[0])

                        self.last_mean_rewards = mean_reward_left[0]
                        self.save(os.path.join(self.nn_dir, self.config['name']))
                        if self.last_mean_rewards > self.config['score_to_win']:
                            print('Network won!')
                            self.save(os.path.join(self.nn_dir, checkpoint_name))
